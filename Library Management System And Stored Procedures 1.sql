...
/*==================== POPULATING TABLES ======================*/
	...
	SELECT * FROM tbl_borrower
INSERT INTO tbl_borrower (borrower_CardNo, borrower_BorrowerName, borrower_BorrowerAddress, borrower_BorrowerPhone)
VALUES ('100', 'John Doe', '123 Main St', '555-1234');
...
SELECT
    SUM(copies.book_copies_No_Of_Copies) AS `Total Copies`
FROM
...
WHERE
    book.book_Title = 'The Lost Tribe'
...
FROM
    tbl_borrower AS borrower
LEFT JOIN
    tbl_book_loans AS book_loans ON borrower.borrower_CardNo = book_loans.book_loans_CardNo
WHERE
    book_loans.book_loans_CardNo IS NULL;
...
WHERE
    branch.library_branch_BranchName = 'Sharpstown'
    AND loans.book_loans_DueDate = CURDATE();
...
SELECT
    branch.library_branch_BranchName AS `Branch Name`,
    COUNT(loans.book_loans_BookID) AS `Total Books Loaned Out`
FROM
    tbl_book_loans AS loans
JOIN
    tbl_library_branch AS branch ON loans.book_loans_BranchID = branch.library_branch_BranchID
...
FROM
    tbl_book AS book
JOIN
    tbl_book_copies AS copies ON book.book_BookID = copies.book_copies_BookID
JOIN
    tbl_book_authors AS authors ON book.book_BookID = authors.book_authors_BookID
JOIN
    tbl_library_branch AS branch ON copies.book_copies_BranchID = branch.library_branch_BranchID
WHERE
    authors.book_authors_AuthorName = 'Stephen King'
